...
class Widget extends \WP_Widget {
	...
	function __construct() {
		$widget_ops = array(
			...
			'description' => __( "Use this widget to add Business / Organization details, contact details, information, location map and a general Manager Contacts form as a widget.", 'manager-contacts-page-contact-people' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct('people_contacts', __('Manager Contacts Widget', 'manager-contacts-page-contact-people' ), $widget_ops);
	}
	...
	public static function widget_contact_form($widget_id = 0){
		...
		if( isset( $_POST['submitted'] ) ) {
				...
				if( trim( $_POST['contactName'] ) === '' ) {
					$nameError =  __( 'You forgot to enter your name.', 'manager-contacts-page-contact-people' );
					$hasError = true;
				} else {
				...
				if( trim( $_POST['email'] ) === '' )  {
					$emailError = __( 'You forgot to enter your email address.', 'manager-contacts-page-contact-people' );
					$hasError = true;
				} else if ( ! filter_var( trim($_POST['email'] ), FILTER_VALIDATE_EMAIL ) ) {
					$emailError = __( 'You entered an invalid email address.', 'manager-contacts-page-contact-people' );
					$hasError = true;
				} else {
				...
				if( trim( $_POST['comments'] ) === '' ) {
					$contactError = __( 'You forgot to enter your comments.', 'manager-contacts-page-contact-people' );
					$hasError = true;
				} else {
				...
				if ( $show_acceptance && ( ! isset( $_POST['agree_terms'] ) || 1 != $_POST['agree_terms'] ) ) {
					$agreeTermsError = people_ict_t__( 'Default Form - Agree Terms Error', __('You need to agree to the website terms and conditions if want to submit this inquiry', 'manager-contacts-page-contact-people' ) );
					$hasError = true;
				}
				...
				if( ! isset( $hasError ) ) {
					...
					$subject = __( 'Contact Form Submission from', 'manager-contacts-page-contact-people' ). ' ' .$name;
		...
		<script type="text/javascript">
				...
				jQuery( 'form#contactForm').on('submit', function() {
					...
					jQuery( '.requiredField').each(function() {
						if(jQuery(this).val().trim() == '') {
							...
							jQuery(this).parent().append( '<span class="error"><?php _e( 'You forgot to enter your', 'manager-contacts-page-contact-people' ); ?> '+labelText+'.</span>' );
							...
							if(!emailReg.test(jQuery(this).val().trim())) {
								var labelText = jQuery(this).prev( 'label').text();
								jQuery(this).parent().append( '<span class="error"><?php _e( 'You entered an invalid', 'manager-contacts-page-contact-people' ); ?> '+labelText+'.</span>' );
								jQuery(this).addClass( 'inputError' );
								hasError = true;
							}
						}
					...
					if ( 0 === is_agree_terms ) {
						agree_terms.parents('li.agree_inline').append( '<span class="error"><?php people_ict_t_e( 'Default Form - Agree Terms Error', __('You need to agree to the website terms and conditions if want to submit this inquiry', 'manager-contacts-page-contact-people' ) ); ?></span>' );
						hasError = true;
					}
					<?php } ?>
					...
					if(!hasError) {
						...
						jQuery.post(jQuery(this).attr( 'action'),formInput, function(data){
							jQuery( 'form#contactForm').slideUp( "fast", function() {
								jQuery(this).before( '<p class="tick"><?php _e( '<strong>Thanks!</strong> Your email was successfully sent.', 'manager-contacts-page-contact-people' ); ?></p>' );
							});
						});
					} else {
            ...
            <div class="people_contact_widget_container">
				...
				if( isset( $emailSent ) && $emailSent == true ) { ?>
					<p class="info">
					  <?php _e( 'Your email was successfully sent.', 'manager-contacts-page-contact-people' ); ?>
					</p>
				<?php } else { ?>
					<div style="clear:both;"></div>
					<div class="location-twitter">
					  <section id="office-location">
						<ul>
						  ...
						  <li>
							<span><img src="<?php echo $phone_icon;?>" /></span><?php _e('Phone:', 'manager-contacts-page-contact-people' ); ?>
						  ...
						  <li>
							<span><img src="<?php echo $fax_icon;?>" /></span><?php _e('Fax:', 'manager-contacts-page-contact-people' ); ?>
						  ...
						  <li>
							<span><img src="<?php echo $mobile_icon;?>" /></span><?php _e('Mobile:', 'manager-contacts-page-contact-people' ); ?>
						  ...
						  <li>
							<span><img src="<?php echo $email_icon;?>" /></span><?php _e('Email:', 'manager-contacts-page-contact-people' ); ?>
					...
					<p class="alert">
					  <?php _e( 'There was an error submitting the form.', 'manager-contacts-page-contact-people' ); ?>
					</p>
					<?php } ?>
					  ...
					  <ol class="forms">
						<li>
						  <label for="contactName">
							<?php _e( 'Name', 'manager-contacts-page-contact-people' ); ?> <span class="require">*</span>
						  </label>
						...
						<li>
						  <label for="email">
							<?php _e( 'Email', 'manager-contacts-page-contact-people' ); ?> <span class="require">*</span>
						  </label>
						...
						<li class="textarea">
						  <label for="commentsText">
							<?php _e( 'Message', 'manager-contacts-page-contact-people' ); ?> <span class="require">*</span>
						  </label>
						...
						<li class="inline">
	                     <label>
						  <span for="sendCopy">
							...
							<?php _e( 'Send a copy of this email to yourself', 'manager-contacts-page-contact-people' ); ?>
						  </span>
						 </label>
	                    </li>
	                    <?php } ?>
						...
						<?php if ( empty( $condition_text ) ) { $condition_text = __( 'I have read and agree to the website terms and conditions', 'manager-contacts-page-contact-people' ); } ?>
						...
						<li class="buttons">
						  ...
						  <input class="submit button" type="submit" value="<?php esc_attr_e( 'Send', 'manager-contacts-page-contact-people' ); ?>" /> <img class="contact-site-ajax-wait" src="<?php echo PEOPLE_CONTACT_IMAGE_URL; ?>/ajax-loader2.gif" border="0" style="display:none; padding:0; margin:0; vertical-align: middle;" />
						</li>
					  </ol>
					</form>
			<?php
...
?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title:', 'manager-contacts-page-contact-people' ); ?></label>
