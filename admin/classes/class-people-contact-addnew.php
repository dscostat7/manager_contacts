...
class AddNew
{
	...
	public static function admin_screen_add_edit() {
		...
		if ( get_option('a3_people_profile_save_failure', 0 ) == 1 ) {
			...
			$message             = '<div class="message error"><p>' . __( 'ERROR: A location address must be entered. Please enter a Location Address for this profile.' , 'manager-contacts-page-contact-people' ) .'</p></div>';
		...
		$title          = __('Add New Profile', 'manager-contacts-page-contact-people' );
		...
		$bt_cancel      = '<input type="button" class="button" onclick="window.location=\'admin.php?page=people-contact-manager\'" value="'.__('Cancel', 'manager-contacts-page-contact-people' ).'" />';
		...
		if (isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['id']) && $_GET['id'] >= 0) {
			...
			$title   = __('Edit Profile', 'manager-contacts-page-contact-people' );
		  ...
		  <div class="contact_manager a3rev_panel_container a3rev_custom_panel_container">
            ...
            <div class="col-left">
				...
				<table class="form-table" style="margin-bottom:0;">
				  <tbody>
					<tr valign="top">
					  <th scope="row"><label for="c_title"><?php _e('Title / Position', 'manager-contacts-page-contact-people' ) ?></label></th>
					...
					<tr valign="top">
					  <th scope="row"><label for="c_name"><?php _e('Name', 'manager-contacts-page-contact-people' ) ?></label></th>
					...
					<tr valign="top">
					  <th scope="row"><label for="c_identitier"><?php _e('ID', 'manager-contacts-page-contact-people' ) ?></label></th>
					  <td><input type="text" maxlength="30" class="regular-text" value="<?php if(isset($data['c_identitier'])){ esc_attr_e( stripslashes( $data['c_identitier']));}?>" id="c_identitier" name="contact_arr[c_identitier]">
					  	<div style="font-size: 13px; font-style: italic;"><?php echo __( 'For Admin identifier, when Profile has multiple Contact cards that have different contact forms. Not shown on front end. Limited to 30 Characters.', 'manager-contacts-page-contact-people' ); ?></div>
					  </td>
					</tr>
					<tr valign="top">
					  <th scope="row"><label for="c_avatar"><?php _e('Profile Image', 'manager-contacts-page-contact-people' ) ?></label></th>
					  <td class="profileavatar">
	                  <?php echo $GLOBALS[PEOPLE_CONTACT_PREFIX.'uploader']->upload_input( 'c_avatar', 'c_avatar', $data['c_avatar'], $data['c_attachment_id'], '', __('Profile Image', 'manager-contacts-page-contact-people' ), '', 'width:100%;', '<div class="description">'.__("Image format .jpg, .png", 'manager-contacts-page-contact-people' ).'</div>' ); ?>
	                  </td>
					</tr>
	        	  </tbody>
				</table>
		        ...
		        $GLOBALS[PEOPLE_CONTACT_PREFIX.'admin_interface']->panel_box( $settings_html, array(
		        	'name' 		=> __( 'Profile Details', 'manager-contacts-page-contact-people' ),
		        	'desc'		=> __("Fields left empty will not show on the front end.", 'manager-contacts-page-contact-people' ),
		        	'css'		=> 'margin-top: 5px',
				...
				<table class="form-table" style="margin-bottom:0;" width="100%">
				  <tbody>
					<tr valign="top">
					  <th scope="row"><label for="c_email"><?php _e('Email', 'manager-contacts-page-contact-people' ) ?></label></th>
					...
					<tr valign="top">
					  <th scope="row"><label for="c_phone"><?php _e('Phone', 'manager-contacts-page-contact-people' ) ?></label></th>
					...
					<tr valign="top">
					  <th scope="row"><label for="c_fax"><?php _e('Fax', 'manager-contacts-page-contact-people' ) ?></label></th>
					...
					<tr valign="top">
					  <th scope="row"><label for="c_mobile"><?php _e('Mobile', 'manager-contacts-page-contact-people' ) ?></label></th>
	                ...
	                <tr valign="top">
					  <th scope="row"><label for="c_website"><?php _e('Website', 'manager-contacts-page-contact-people' ) ?></label></th>
	                ...
	                <tr valign="top">
					  <td class="forminp" colspan="2" style="padding-left:0;">
					   <div class="row_mod" ><label for="c_website"><?php _e('About', 'manager-contacts-page-contact-people' ) ?></label></div>
		        ...
		        $settings_html = ob_get_clean();
		        $GLOBALS[PEOPLE_CONTACT_PREFIX.'admin_interface']->panel_box( $settings_html, array(
		        	'name' 		=> __( 'Contact Details', 'manager-contacts-page-contact-people' ),
		        	'desc'		=> __("Fields left empty will not show on the front end.", 'manager-contacts-page-contact-people' ),
				...
				<table class="form-table" style="margin-bottom:0;">
				  <tbody>
					<tr valign="top">
					  <th scope="row"><label for="show_on_main_page"><?php _e('Contact Page Display', 'manager-contacts-page-contact-people' ) ?></label></th>
					  <td>
					  	<?php
					  	...
					  	<input
	                        ...
	                        checked_label="<?php _e( 'ON', 'manager-contacts-page-contact-people' ); ?>"
	                        unchecked_label="<?php _e( 'OFF', 'manager-contacts-page-contact-people' ); ?>"
	                        type="checkbox"
							value="1"
				...
				<div class="show_marker_on_main_map_container" style="<?php if ( 1 != $data['show_on_main_page'] ) { echo 'visibility:hidden; height: 0px; overflow: hidden; margin-bottom: 0px;'; } ?>" >
					<table class="form-table" style="margin-bottom:0;">
					  <tbody>
						<tr valign="top">
						  <th scope="row"><label for="enable_map_marker"><?php _e('Show on Contact Page map', 'manager-contacts-page-contact-people' ) ?></label></th>
						  <td>
						  	<?php
						  	...
						  	<input
								...
								/>
								<span class="description"><?php echo __( 'ON to show this Profile on the Contact Page map', 'manager-contacts-page-contact-people' ); ?></span>
						  </td>
						</tr>
					  </tbody>
					</table>
		        ...
		        $settings_html = ob_get_clean();
		        $GLOBALS[PEOPLE_CONTACT_PREFIX.'admin_interface']->panel_box( $settings_html, array(
		        	'name' 		=> __( 'Contact Page Profile', 'manager-contacts-page-contact-people' ),
		        	'desc'		=> __( 'Switch OFF and this profile will not show on the main Manager Contacts Page but still can be inserted by shortcode (Pro and Ultimate Versions) and assigned to groups (Ultimate Version) that are inserted by shortcode.', 'manager-contacts-page-contact-people' ),
			...
			<div class="col-right">
				...
				<table class="form-table" style="margin-bottom:0;">
				  <tbody>
					<tr valign="top">
					  <th scope="row"><label for="c_address"><?php _e('Address', 'manager-contacts-page-contact-people' ) ?></label></th>
					...
					<tr valign="top">
					  <th scope="row"><label for="c_latitude"><?php _e('Latitude', 'manager-contacts-page-contact-people' ) ?></label></th>
					...
					<tr valign="top">
					  <th scope="row"><label for="c_longitude"><?php _e('Longitude', 'manager-contacts-page-contact-people' ) ?></label></th>
					...
					<tr>
						<th></th>
						<td>
							<div style="font-size: 13px;"><?php echo __( '* <strong>Tip</strong> - drag and drop the map marker to the required location', 'manager-contacts-page-contact-people' ); ?></div>
		        ...
		        $settings_html = ob_get_clean();
		        $GLOBALS[PEOPLE_CONTACT_PREFIX.'admin_interface']->panel_box( $settings_html, array(
		        	'name' 		=> __( 'Profile Location Address', 'manager-contacts-page-contact-people' ),
		        	'desc'		=> __( '<strong>REQUIRED</strong>: All profiles must have a map location set. The location set here is used for this profile on the Manager Contacts Page map (optional) and any Group map (optional) that the profile has been assigned to.', 'manager-contacts-page-contact-people' ),
