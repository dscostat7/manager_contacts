...
class Admin_UI
{
	...
	public $plugin_prefix = PEOPLE_CONTACT_PREFIX;
	...
	public function __construct() {
		...
		$this->support_url = 'https://wordpress.org/support/plugin/manager-contacts-page-contact-people/';
	}
	...
	public function get_premium_video_data() {
		$premium_video_data = array(
				'box_title'    => __( 'Premium Version Enhanced Features', 'manager-contacts-page-contact-people' ),
				'image_url'    => PEOPLE_CONTACT_IMAGE_URL. '/video.jpg',
				'video_url'    => 'https://www.youtube.com/embed/9dGw-ORfMIk?version=3&autoplay=1',
				'left_title'   => __( 'Premium Version Enhanced Features', 'manager-contacts-page-contact-people' ),
				'left_text'    => __( 'Contact People Ultimate', 'manager-contacts-page-contact-people' )
									. "\n\n" . __( 'Quick Video showing the main (not all) enhanced features that are built into the Contact People Ultimate version', 'manager-contacts-page-contact-people' ),
				'right_title'  => __( 'Developer Support and Premium Features', 'manager-contacts-page-contact-people' ),
				'right_text'   => __( 'Limited Time Offer. Purchase the Premium Version Lifetime License. That is a Lifetime of maintenance updates, feature upgrades and developer support for a once only fee. Offer ending soon.', 'manager-contacts-page-contact-people' )
	...
	public function upgrade_top_message( $echo = false, $setting_id = '' ) {
		$upgrade_top_message = sprintf( '<div class="pro_feature_top_message">'
			. __( 'Advanced Settings - Upgrade to the <a href="%s" target="_blank">%s</a> to activate these settings.', 'manager-contacts-page-contact-people' )
			. '</div>'
			...
			, apply_filters( $this->plugin_name . '_' . $setting_id . '_pro_version_name', apply_filters( $this->plugin_name . '_pro_version_name', __( 'Ultimate Version', 'manager-contacts-page-contact-people' ) ) )
		);
	...
	public function get_version_message() {
		...
		if ( false !== $version_transient ) {
			...
			if ( false === $timeout ) {
				$version_message = __( 'You should check now to see if have any new version is available', 'manager-contacts-page-contact-people' );
			} elseif ( 'cannot_connect_api' == $version_transient ) {
				$version_message = sprintf( __( 'Connection Failure! Please try again. If this issue persists please create a support request on the plugin <a href="%s" target="_blank">a3rev support forum</a>.', 'manager-contacts-page-contact-people' ), $this->support_url );
			} else {
				...
				if ( FALSE !== stristr( $version_transient, '||' )
					...
					&& version_compare( PEOPLE_CONTACT_VERSION , $version_info[0], '<' ) ) {
						...
						$version_message = sprintf( __( 'There is a new version <span class="a3rev-ui-new-plugin-version">%s</span> available, <a href="%s" target="_blank">update now</a> or download direct from <a href="%s" target="_blank">My Account</a> on a3rev.com', 'manager-contacts-page-contact-people' ),
							$version_info[0],
