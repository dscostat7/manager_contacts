...
class Admin_Interface extends Admin_UI
{
			...
			switch ( trim( sanitize_text_field( wp_unslash( $_REQUEST['type'] ) ) ) ) {
				...
				case 'check_new_version':
					...
					if ( '' == trim( $version_message ) ) {
						$version_message = __( 'Great! You have the latest version installed.', 'manager-contacts-page-contact-people' );
						$has_new_version = 0;
					} else {
	...
	public function get_success_message( $message = '' ) {
		if ( trim( $message ) == '' ) $message = __( 'Settings successfully saved.' , 'manager-contacts-page-contact-people' ); 
	...
	public function get_error_message( $message = '' ) {
		if ( trim( $message ) == '' ) $message = __( 'Error: Settings can not save.' , 'manager-contacts-page-contact-people' ); 
	...
	public function get_reset_message( $message = '' ) {
		if ( trim( $message ) == '' ) $message = __( 'Settings successfully reseted.' , 'manager-contacts-page-contact-people' ); 
		return '<div class="updated" id=""><p>' . $message . '</p></div>';
	}
	...
	public function get_font_weights() {
		$font_weights = array (
			'300'				=> __( 'Thin', 'manager-contacts-page-contact-people' ),
			'300 italic'		=> __( 'Thin/Italic', 'manager-contacts-page-contact-people' ),
			'normal'			=> __( 'Normal', 'manager-contacts-page-contact-people' ),
			'italic'			=> __( 'Italic', 'manager-contacts-page-contact-people' ),
			'bold'				=> __( 'Bold', 'manager-contacts-page-contact-people' ),
			'bold italic'		=> __( 'Bold/Italic', 'manager-contacts-page-contact-people' ),
		);
	...
	public function get_border_styles() {
		$border_styles = array (
			'solid'				=> __( 'Solid', 'manager-contacts-page-contact-people' ),
			'double'			=> __( 'Double', 'manager-contacts-page-contact-people' ),
			'dashed'			=> __( 'Dashed', 'manager-contacts-page-contact-people' ),
			'dotted'			=> __( 'Dotted', 'manager-contacts-page-contact-people' ),
			'groove'			=> __( 'Groove', 'manager-contacts-page-contact-people' ),
			'ridge'				=> __( 'Ridge', 'manager-contacts-page-contact-people' ),
			'inset'				=> __( 'Inset', 'manager-contacts-page-contact-people' ),
			'outset'			=> __( 'Outset', 'manager-contacts-page-contact-people' ),
		);
	...
	public function admin_forms( $options, $form_key, $option_name = '', $form_messages = array() ) {
		...
		foreach ( $options as $value ) {
			...
			switch( $value['type'] ) {
				...
				case 'google_api_key':
					...
					if ( ! isset( $value['checked_label'] ) ) $value['checked_label'] = __( 'ON', 'manager-contacts-page-contact-people' );
					if ( ! isset( $value['unchecked_label'] ) ) $value['unchecked_label'] = __( 'OFF', 'manager-contacts-page-contact-people' );

					?><tr valign="top">
						<th scope="row" class="titledesc">
                        	<?php echo wp_kses_post( $tip ); ?>
							<label for="<?php echo esc_attr( $this->google_api_key_option ); ?>"><?php esc_html_e( 'Google Fonts API', 'manager-contacts-page-contact-people' ); ?></label>
						</th>
						<td class="forminp forminp-onoff_checkbox forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
							<input
								...
								/> <span class="description" style="margin-left:5px;"><?php esc_html_e( 'ON to connect to Google Fonts API and have auto font updates direct from Google.', 'manager-contacts-page-contact-people' ); ?></span>
				...
				case 'google_map_api_key':
					...
					?><tr valign="top">
						<th scope="row" class="titledesc">
                        	<?php echo wp_kses_post( $tip ); ?>
							<label for="<?php echo esc_attr( $this->google_map_api_key_option ); ?>"><?php esc_html_e( 'Google Maps API', 'manager-contacts-page-contact-people' ); ?></label>
						</th>
						<td class="forminp forminp-onoff_checkbox forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
							<input
								...
								/> <span class="description" style="margin-left:5px;"><?php esc_html_e( 'Switch ON to connect to Google Maps API', 'manager-contacts-page-contact-people' ); ?></span>
				...
				case 'manual_check_version':
					...
					if ( is_multisite() ) {
						$version_message = __( 'Sorry, this feature just for Network Admin.', 'manager-contacts-page-contact-people' );
					} else {
						global $a3_dashboard_plugin_requirement;

						if ( ! $a3_dashboard_plugin_requirement->is_installed() ) {
							$version_message = sprintf( __( 'You need to install and activate the <a title="" href="%s" target="_parent">a3rev Dashboard plugin</a> for manage version and get auto upgrade to latest version.', 'manager-contacts-page-contact-people' ), $a3_dashboard_plugin_requirement->install_url() );
						} elseif ( ! $a3_dashboard_plugin_requirement->is_activated() ) {
							$version_message = sprintf( __( 'You need to activate the <a title="" href="%s" target="_parent">a3rev Dashboard plugin</a> for manage version and get auto upgrade to latest version.', 'manager-contacts-page-contact-people' ), $a3_dashboard_plugin_requirement->activate_url() );
						} elseif ( function_exists( 'is_a3_club_membership' ) && ! is_a3_club_membership() ) {
							$version_message = sprintf( __( 'You need to go to <a title="" href="%s">a3 Dashboard Main page</a> for login before check for Update. Use your account creds on <a href="https://a3rev.com" target="_parent">a3rev.com</a> to login.', 'manager-contacts-page-contact-people' ), self_admin_url( 'admin.php?page=a3rev-dashboard' ) );
						} else {
							$check_version_class = 'a3rev-ui-latest-version-message';
							$version_message = sprintf( __( 'Go to <a href="%s">a3 Dashboard Main page</a> and hit on <strong>CHECK NOW</strong> button to manual check for Update.', 'manager-contacts-page-contact-people' ), self_admin_url( 'admin.php?page=a3rev-dashboard' ) );
						}
					}
				...
				case 'onoff_toggle_box' :
					...
					?><tr valign="top">
						<th scope="row" class="titledesc">
                        	<?php echo wp_kses_post( $tip ); ?>
							<label for="<?php echo esc_attr( $this->toggle_box_open_option ); ?>"><?php esc_html_e( 'Open Box Display', 'manager-contacts-page-contact-people' ); ?></label>
						</th>
						<td class="forminp forminp-onoff_checkbox forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
							<input
								...
								<?php $this->esc_attribute_array_e( $custom_attributes ); // WPCS: XSS ok. ?>
								/> <span class="description" style="margin-left:5px;"><?php esc_html_e( 'ON and each admin panel setting box OPEN | CLOSED position are saved each time changes are SAVED.', 'manager-contacts-page-contact-people' ); ?></span>
                        </td>
					</tr><?php
				break;
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
							<input
									...
									checked_label="<?php esc_attr_e( 'ON', 'manager-contacts-page-contact-people' ); ?>"
									unchecked_label="<?php esc_attr_e( 'OFF', 'manager-contacts-page-contact-people' ); ?>"
									type="checkbox"
									value="1"
							...
							<fieldset>
								...
								if ( is_array( $value['onoff_options'] ) && count( $value['onoff_options'] ) > 0 ) {
									foreach ( $value['onoff_options'] as $i_option ) {
										if ( ! isset( $i_option['checked_label'] ) ) $i_option['checked_label'] = __( 'ON', 'manager-contacts-page-contact-people' );
										if ( ! isset( $i_option['unchecked_label'] ) ) $i_option['unchecked_label'] = __( 'OFF', 'manager-contacts-page-contact-people' );
				...
				case 'image_size' :
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
							...
							<label><?php esc_html_e( 'Width', 'manager-contacts-page-contact-people' ); ?> <input name="<?php $this->esc_attribute_name_e( $name_attribute ); ?>[width]" id="<?php echo esc_attr( $id_attribute ); ?>-width" type="text" class="a3rev-ui-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>-width" value="<?php echo esc_attr( $width ); ?>" /></label>
	
							<label><?php esc_html_e( 'Height', 'manager-contacts-page-contact-people' ); ?> <input name="<?php $this->esc_attribute_name_e( $name_attribute ); ?>[height]" id="<?php echo esc_attr( $id_attribute ); ?>-height" type="text" class="a3rev-ui-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>-height" value="<?php echo esc_attr( $height ); ?>" /></label>
	
							<label><?php esc_html_e( 'Hard Crop', 'manager-contacts-page-contact-people' ); ?> <input name="<?php $this->esc_attribute_name_e( $name_attribute ); ?>[crop]" id="<?php echo esc_attr( $id_attribute ); ?>-crop" type="checkbox" class="a3rev-ui-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>-crop" <?php echo checked( 1, $option_value['crop'], false ); ?> /></label>
						...
						<td class="forminp">
                           ...
                           <!-- Font Face -->
							<select
								...
								>
								<optgroup label="<?php esc_attr_e( '-- Default Fonts --', 'manager-contacts-page-contact-people' ); ?>">
                                <?php
                                ...
                                <optgroup label="<?php esc_attr_e( '-- Google Fonts --', 'manager-contacts-page-contact-people' ); ?>">
                                <?php
                           ...
                           <!-- Preview Button -->
                           <div class="a3rev-ui-typography-preview"><a href="#" class="a3rev-ui-typography-preview-button button submit-button" title="<?php esc_attr_e( 'Preview your customized typography settings', 'manager-contacts-page-contact-people'); ?>"><span>&nbsp;</span></a></div>
				...
				case 'border':
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-border_corner">
                           ...
                           <!-- Preview Button -->
                           <div class="a3rev-ui-settings-preview"><a href="#" class="a3rev-ui-border-preview-button a3rev-ui-settings-preview-button button submit-button" title="<?php esc_attr_e( 'Preview your customized border settings', 'manager-contacts-page-contact-people' ); ?>"><span>&nbsp;</span></a></div>
                           <span class="description" style="margin-left:5px;"><?php esc_html_e( '0px = No Border', 'manager-contacts-page-contact-people' ); ?></span>
                           ...
                           <!-- Border Corner : Rounded or Square -->
								<input
                                    ...
                                    checked_label="<?php esc_attr_e( 'Rounded', 'manager-contacts-page-contact-people' ); ?>"
                                    unchecked_label="<?php esc_attr_e( 'Square', 'manager-contacts-page-contact-people' ); ?>"
                                    type="checkbox"
                                    value="rounded"
							...
							<!-- Border Rounded Value -->
								<div class="a3rev-ui-border-corner-value-container">
                                	<div class="a3rev-ui-border_corner-top_left">
                                        <span class="a3rev-ui-border_corner-span"><?php esc_html_e( 'Top Left Corner', 'manager-contacts-page-contact-people' ); ?></span>
                                    ...
                                    <div class="a3rev-ui-border_corner-top_right">
                                        <span class="a3rev-ui-border_corner-span"><?php esc_html_e( 'Top Right Corner', 'manager-contacts-page-contact-people' ); ?></span> 
                                        <div class="a3rev-ui-slide-container">
                                            <div class="a3rev-ui-slide-container-start">
                                                <div class="a3rev-ui-slide-container-end">
                                    ...
                                    <div class="a3rev-ui-border_corner-bottom_right">
                                        <span class="a3rev-ui-border_corner-span"><?php esc_html_e( 'Bottom Right Corner', 'manager-contacts-page-contact-people' ); ?></span> 
                                        <div class="a3rev-ui-slide-container">
                                            <div class="a3rev-ui-slide-container-start">
                                                <div class="a3rev-ui-slide-container-end">
                                    ...
                                    <div class="a3rev-ui-border_corner-bottom_left">
                                        <span class="a3rev-ui-border_corner-span"><?php esc_html_e( 'Bottom Left Corner', 'manager-contacts-page-contact-people' ); ?></span>
                                        <div class="a3rev-ui-slide-container"> 
                                            <div class="a3rev-ui-slide-container-start">
                                                <div class="a3rev-ui-slide-container-end">
				...
				case 'border_styles':
					...
					?><tr valign="top">
						...
						<td class="forminp">
                           ...
                           <!-- Preview Button -->
                           <div class="a3rev-ui-settings-preview"><a href="#" class="a3rev-ui-border-preview-button a3rev-ui-settings-preview-button button submit-button" title="<?php esc_attr_e( 'Preview your customized border styles settings', 'manager-contacts-page-contact-people' ); ?>"><span>&nbsp;</span></a></div>
                           <span class="description" style="margin-left:5px;"><?php esc_html_e( '0px = No Border', 'manager-contacts-page-contact-people' ); ?></span>
                           </div>
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-box_shadow">
							...
							<select
                                ...
                                data-placeholder="<?php esc_attr_e( 'Horizontal Shadow', 'manager-contacts-page-contact-people' ); ?>"
								>
								<?php
									for ( $i = -20; $i <= 20; $i++ ) {
										?>
							...
							<select
                                ...
                                data-placeholder="<?php esc_attr_e( 'Vertical Shadow', 'manager-contacts-page-contact-people' ); ?>"
								>
								<?php
									for ( $i = -20; $i <= 20; $i++ ) {
										?>
                           ...
                           <!-- Box Blur Distance -->
							<select
                                ...
                                data-placeholder="<?php esc_attr_e( 'Blur Distance', 'manager-contacts-page-contact-people' ); ?>"
								>
								<?php
									for ( $i = 0; $i <= 20; $i++ ) {
										?>
                           ...
                           <!-- Box Spread -->
							<select
                                ...
                                data-placeholder="<?php esc_attr_e( 'Spread Size', 'manager-contacts-page-contact-people' ); ?>"
								>
								<?php
									for ( $i = 0; $i <= 20; $i++ ) {
										?>
                           ...
                           <!-- Box Shadow Inset -->
                                <input
                                    ...
                                    checked_label="<?php esc_attr_e( 'INNER', 'manager-contacts-page-contact-people' ); ?>"
                                    unchecked_label="<?php esc_attr_e( 'OUTER', 'manager-contacts-page-contact-people' ); ?>"
                                    type="checkbox"
                                    value="inset"
                           ...
                           <input
                            ...
                            <!-- Preview Button -->
                           <div class="a3rev-ui-settings-preview"><a href="#" class="a3rev-ui-box_shadow-preview-button a3rev-ui-settings-preview-button button submit-button" title="<?php esc_attr_e( 'Preview your customized box shadow settings', 'manager-contacts-page-contact-people'); ?>"><span>&nbsp;</span></a></div>   
                           </div>
                           <div style="clear:both;"></div>
                           </div>
						</td>
		...
		<?php do_action( $this->plugin_name . '-' . trim( $form_key ) . '_settings_end' ); ?>
            <p class="submit">
                    ...
                    <input type="submit" value="<?php esc_attr_e('Save changes', 'manager-contacts-page-contact-people'); ?>" class="button button-primary" name="bt_save_settings" />
                    <input type="submit" name="bt_reset_settings" class="button" value="<?php esc_attr_e('Reset Settings', 'manager-contacts-page-contact-people'); ?>"  />
