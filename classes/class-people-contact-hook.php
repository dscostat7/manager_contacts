...
class Hook_Filter
{
	public static function register_admin_screen () {
		...
		$contact_manager = add_menu_page( __('Manager Contacts', 'manager-contacts-page-contact-people' ), __('Manager Contacts', 'manager-contacts-page-contact-people' ), 'manage_options', 'people-contact-manager', array( __NAMESPACE__ . '\Admin\Profile_Manager', 'admin_screen' ), null, '27.222');

		$profile = add_submenu_page('people-contact-manager', __( 'Profiles', 'manager-contacts-page-contact-people' ), __( 'Profiles', 'manager-contacts-page-contact-people' ), 'manage_options', 'people-contact-manager', array( __NAMESPACE__ . '\Admin\Profile_Manager', 'admin_screen' ) );

		$add_new = add_submenu_page('people-contact-manager', __( 'Add New Profile', 'manager-contacts-page-contact-people' ), __( 'Add New Profile', 'manager-contacts-page-contact-people' ), 'manage_options', 'people-contact', array( __NAMESPACE__ . '\Admin\AddNew', 'admin_screen_add_edit' ) );

		$categories_page = add_submenu_page('people-contact-manager', __( 'Groups', 'manager-contacts-page-contact-people' ), __( 'Groups', 'manager-contacts-page-contact-people' ), 'manage_options', 'people-category-manager', array( __NAMESPACE__ . '\Admin\Category_Manager', 'admin_screen' ) );
	...
	public static function footer_default_form_scripts() {
		...
		wp_localize_script( 'people-ei-default-form',
			...
			apply_filters( 'people_ei_default_vars', array(
				...
				'email_valid_error' => people_ict_t__( 'Default Form - Contact Email Error', __( 'Please enter valid Email address', 'manager-contacts-page-contact-people' ) ),
				'required_error'    => people_ict_t__( 'Default Form - Required Error', __( 'is required', 'manager-contacts-page-contact-people' ) ),
				'agree_terms_error' => people_ict_t__( 'Default Form - Agree Terms Error', __( 'You need to agree to the website terms and conditions if want to submit this inquiry', 'manager-contacts-page-contact-people' ) ),
	...
	public static function plugin_extension_box( $boxes = array() ) {
		...
		$support_box = '<a href="'.$GLOBALS[PEOPLE_CONTACT_PREFIX.'admin_init']->support_url.'" target="_blank" alt="'.__('Go to Support Forum', 'manager-contacts-page-contact-people' ).'"><img src="'.PEOPLE_CONTACT_IMAGE_URL.'/go-to-support-forum.png" /></a>';
		$boxes[] = array(
			'content' => $support_box,
			'css' => 'border: none; padding: 0; background: none;'
		);

		$free_wordpress_box = '<a href="https://profiles.wordpress.org/a3rev/#content-plugins" target="_blank" alt="'.__('Free WordPress Plugins', 'manager-contacts-page-contact-people' ).'"><img src="'.PEOPLE_CONTACT_IMAGE_URL.'/free-wordpress-plugins.png" /></a>';
		$boxes[] = array(
			'content' => $free_wordpress_box,
			'css' => 'border: none; padding: 0; background: none;'
		);

        $rating_box = '<div style="margin-bottom: 5px; font-size: 12px;"><strong>' . __('Is this plugin is just what you needed? If so', 'manager-contacts-page-contact-people' ) . '</strong></div>';
        $rating_box .= '<a href="https://wordpress.org/support/view/plugin-reviews/manager-contacts-page-contact-people?filter=5#postform" target="_blank" alt="'.__('Submit Review for Plugin on WordPress', 'manager-contacts-page-contact-people' ).'"><img src="'.PEOPLE_CONTACT_IMAGE_URL.'/a-5-star-rating-would-be-appreciated.png" /></a>';
        $boxes[] = array(
            'content' => $rating_box,
            'css' => 'border: none; padding: 0; background: none;'
        );

        $follow_box = '<div style="margin-bottom: 5px;">' . __('Connect with us via', 'manager-contacts-page-contact-people' ) . '</div>';
		$follow_box .= '<a href="https://www.facebook.com/a3rev" target="_blank" alt="'.__('a3rev Facebook', 'manager-contacts-page-contact-people' ).'" style="margin-right: 5px;"><img src="'.PEOPLE_CONTACT_IMAGE_URL.'/follow-facebook.png" /></a> ';
		$follow_box .= '<a href="https://twitter.com/a3rev" target="_blank" alt="'.__('a3rev Twitter', 'manager-contacts-page-contact-people' ).'"><img src="'.PEOPLE_CONTACT_IMAGE_URL.'/follow-twitter.png" /></a>';
		$boxes[] = array(
			'content' => $follow_box,
	...
	public static function plugin_extra_links($links, $plugin_name) {
		...
		$links[] = '<a href="'.$GLOBALS[PEOPLE_CONTACT_PREFIX.'admin_init']->support_url.'" target="_blank">'.__('Support', 'manager-contacts-page-contact-people' ).'</a>';
		return $links;
	}

	public static function settings_plugin_links($actions) {
		$actions = array_merge( array( 'settings' => '<a href="admin.php?page=people-contact-settings">' . __( 'Settings', 'manager-contacts-page-contact-people' ) . '</a>' ), $actions );
	...
	?>
		<div class="error below-h2" style="display:block !important; margin-left:2px;">
			<p><?php echo sprintf( __( 'Warning: No Google Maps API key was found - Maps may not show without a key. Go to the <a href="%s">Google Maps API option box</a>, enter your key and Save Changes or switch Google Maps OFF <a href="%s">here</a> and <a href="%s">here</a>.' , 'manager-contacts-page-contact-people' ), admin_url( 'admin.php?page=people-contact-settings&box_open=google_map_api_key_settings_box' ), admin_url( 'admin.php?page=people-contact-settings&box_open=google_map_settings_box' ), admin_url( 'admin.php?page=people-contact-settings&tab=contact-widget&box_open=contact_widget_map_settings_box' ) ); ?></p>
		</div>
	<?php
	}
}
