...
class Ajax
{
	...
	public function email_inquiry_submit_form_ajax() {
		...
		$json_var = array(
			'status'  => 'error',
			'message' => people_ict_t__( 'Default Form - Contact Not Allow', __( "Sorry, you can't contact at this time.", 'manager-contacts-page-contact-people' ) ),
		);
		...
		if ( '' != trim( $your_subject ) ) {
			$subject = trim( $your_subject ). ' ' . people_ict_t__( 'Email Inquiry - from', __('from', 'manager-contacts-page-contact-people' ) ) . ' ' . ( function_exists('icl_t') ? icl_t( 'WP',__('Blog Title','wpml-string-translation'), get_option('blogname') ) : get_option('blogname') );
		} else {
			$subject = people_ict_t__( 'Email Inquiry - Contact from', __('Contact from', 'manager-contacts-page-contact-people' ) ).' '. ( function_exists('icl_t') ? icl_t( 'WP',__('Blog Title','wpml-string-translation'), get_option('blogname') ) : get_option('blogname') );
		}
