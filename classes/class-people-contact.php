...
class Main {
	...
	public function default_contact_form( $contact_id , $from_page_id = 0 ) {
		...
		else $inquiry_contact_text_button = __('SEND', 'manager-contacts-page-contact-people' );
		...
		$name_label      = people_ict_t__( 'Default Form - Contact Name', __( 'Name', 'manager-contacts-page-contact-people' ) );
		$email_label     = people_ict_t__( 'Default Form - Contact Email', __( 'Email', 'manager-contacts-page-contact-people' ) );
		$phone_label     = people_ict_t__( 'Default Form - Contact Phone', __( 'Phone', 'manager-contacts-page-contact-people' ) );
		$subject_label   = people_ict_t__( 'Default Form - Contact Subject', __( 'Subject', 'manager-contacts-page-contact-people' ) );
		$message_label   = people_ict_t__( 'Default Form - Contact Message', __( 'Message', 'manager-contacts-page-contact-people' ) );
		$send_copy_label = people_ict_t__( 'Default Form - Send Copy', __( 'Send a copy of this email to myself.', 'manager-contacts-page-contact-people' ) );
	...
	public function create_contact_maps($contacts = array() ) {
		...
		if(is_array($contacts) && count($contacts) > 0 ){
			foreach($contacts as $key=>$value){
				...
				$html .= '<p style="margin-bottom:5px;"><span class="p_icon_website"><img src="'.$website_icon.'" style="width:auto;height:auto" /></span> <a rel="noopener" href="'. esc_url( stripslashes($value['c_website'] ) ).'" target="_blank">'.( function_exists('icl_t') ? icl_t( 'a3 Contact People', 'Profile Cards - Website Link Text', __('Visit Website', 'manager-contacts-page-contact-people' ) ) : __('Visit Website', 'manager-contacts-page-contact-people' ) ).'</a></p>';
				}
